...
### the server will be added to list and then all of servers which has more than 40 hogging threads will be sent to L2 team to check 
### the cause of this issue and operate accordingly 
...
def getStuckThreads() : 
	...
	f.write('\n \n @L3 team : Thread dumps are taken under the following path /opt/bea/td please extract them to take a look at this issue ') 
	f.close()
	if flag == 1 :
		
        	os.system('/bin/mailx -s "Critical server stuck threads exceed max count , servers restart"  RECEIVER  -- -f SENDER  < Stuck_Thread.txt')
